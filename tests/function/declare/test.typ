...
#let kinetic-energy = equation.calculate(kinetic-energy-formula)
#kinetic-energy(m1: zi.kg("75.0"), v: zi.m-s("1.50"))

#let circle-area-formula = $pi r^2$
#circle-area-formula
#let circle-area = equation.calculate(circle-area-formula)
...
#arrhenius-equation
#let arrhenius = equation.calculate(arrhenius-equation).with(R: universal-gas-constant)
...
#let sättigungs-molenbruch = equation.calculate(sättigungs-molenbruch-gleichung).with(R: universal-gas-constant)
