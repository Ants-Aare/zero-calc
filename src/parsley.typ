...
#let eval-pariman-node((head, args, slots)) = {
  import calculation: *
  if head == "add" { add(..args) } else if head == "sub" { sub(..args) } else if head == "mul" { mul(..args) } else if (
    head == "frac"
  ) { div(slots.num, slots.denom) } else if head == "pow" { pow(..args) } else if head == "group" {
    slots.expr
  } else if head == "sqrt" { root(slots.radicand, 2) } else if head == "root" {
    root(slots.radicand, slots.index.value)
  } else { panic(head) }
}
  ...
  parsely.walk(
    ...
    leaf: it => {
      if repr(it.func()) == "symbol" {
        if it.text in vars { vars.at(it.text) } else { panic("cannot evaluate symbol", it) }
      } else if it.func() == text {
        if it.text in vars { vars.at(it.text) } else { exact(eval(it.text)) } // try to eval as number
